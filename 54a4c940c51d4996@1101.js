...
select(["Norte", "Nordeste", "Centro-Oeste", "Sudeste", "Sul", "Top5+", "Top5-", "Todos"])
)});
  ...
  main.variable(observer("br_map_view")).define("br_map_view", ["chosenData","vl","br_topo","selected","casesLast","width"], function(chosenData,vl,br_topo,selected,casesLast,width)
{
  ...
  .encode(
    vl.color().fieldQ('deathsPer100k').scale({domain: [100, 400], type: 'quantize', clamp: true, scheme: {name: mapcolor, count: 10}}).legend({orient: 'bottom'}).title('Mortes por 100 mil habitantes'),
    vl.tooltip(['state', {field: 'deathsPer100k', format: '.1f'}])
  )
  ...
  .encode(
    vl.color().fieldQ('casesPer100k').scale({domain: [4000, 21000], type: 'quantize', clamp: true, scheme: {name: mapcolor, count: 10}}).legend({orient: 'bottom'}).title('Casos por 100 mil habitantes'),
    vl.tooltip(['state', {field: 'casesPer100k', format: '.1f'}])
  )
  ...
  main.variable(observer("scatter")).define("scatter", ["vl","color","casesLast","width"], function(vl,color,casesLast,width)
{
  ...
  main.variable(observer("plot2")).define("plot2", ["vl","selected_data","cases","selected","width"], function(vl,selected_data,cases,selected,width)
{
  ...
  const line = vl.markLine().encode(
    ...
    vl.y().fieldQ(selected_data), vl.color().fieldN("state"),
